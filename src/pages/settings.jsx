...
class Settings extends Component {
    ...
    saveSettings = () => {
        ...
        this.notifySync('Apollo配置同步完成', 'success');
    }
    ...
    clearSettings = () => {
        ...
        this.notifySync('Apollo配置重置完成', 'error');
    }
        ...
        return(
            <>
                ...
                <Input.Password label="apollo身份码" value={this.state.authCode} onChange={(e)=>{
                    this.setState({authCode: e.target.value})}} width="85%" maxLength={128}/>
                <Spacer h={.5} />
                <Input label="心跳定时器间隔" placeholder="5" value={this.state.heartbeat} onChange={(e)=>{
                    this.setState({heartbeat: e.target.value})}} width="85%"/>
                <Spacer h={.5} />
                ...
                <Checkbox checked={this.state.moreToast} onChange={(e)=>{
                    this.setState({moreToast: e.target.checked})}}>使用详细通知</Checkbox>
                <Spacer h={.5} />
                <Checkbox checked={this.state.logConsole} onChange={(e)=>{
                    this.setState({logConsole: e.target.checked})}}>日志输出到控制台</Checkbox>
                <Spacer h={.5} />
                <Checkbox checked={this.state.enableHeart} onChange={(e)=>{
                    this.setState({enableHeart: e.target.checked})}}>开启心跳提示</Checkbox>
                <Spacer h={.5} />
                <Checkbox checked={this.state.enableWS} onChange={(e)=>{
                    this.setState({enableWS: e.target.checked})}}>使用websocket通信</Checkbox>
                <Spacer h={.5} />
                <Checkbox checked={this.state.autoHide} onChange={(e)=>{
                    this.setState({autoHide: e.target.checked})}}>自动隐藏无权限页面</Checkbox>
                <Spacer h={.5} />
                <Checkbox checked={this.state.enableAppSpy} onChange={(e)=>{
                    this.setState({enableAppSpy: e.target.checked})}}>监控微服务状态</Checkbox>
                <Spacer h={2} />

                <Grid.Container gap={2}>
                    <Grid><Button auto shadow type="secondary" scale={0.75} onClick={this.saveSettings}>保存</Button></Grid>
